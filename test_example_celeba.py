...
class Generator_light(nn.Module):
    def __init__(self, img_size, in_channel=3):
        ...
        self.l1 = nn.Sequential(nn.Linear(img_size*img_size*in_channel, (img_size)*(img_size)*in_channel),
                                ...
                                nn.LeakyReLU(0.2, inplace=True),
                                nn.Linear(img_size*img_size*in_channel, (img_size)*(img_size)*in_channel+3))
    ...
    def forward(self, x):
        ...
        out = self.l1(x_flat)
        out_img = out[:, :-3]
        out_scale = out[:, -3:].unsqueeze(-1).unsqueeze(-1).unsqueeze(-1)
...
class MaskBlockPhase(nn.Module):
    def __init__(self, 
        ...
        self.register_parameter('mask_phase',
                                nn.Parameter(torch.Tensor(1, 3, mask_x_num, mask_y_num),
                                             requires_grad=True))
...
class D2nnModel(nn.Module):
    ...
    def forward(self, x):
        ...
        img_cplx = torch.square(torch.abs(img_cplx)) # complex amplitude to intensity
...
class TestConfig:
    ...
    c = 299792458 
    ridx_air = 1.0
    wlength_vc = [638e-9, 520e-9, 450e-9]
    freq = [c / 638e-9, c / 520e-9, c / 450e-9] 
    ...
    load_pretrain = True
    ckpt_to_load = './ckpt/ckpt_celeba.pth'
...
def main():
    ...
    generator_e = Generator_light(img_size=config.img_size, in_channel=config.in_channel)
    ...
    with torch.no_grad():
        ...
        d2nn_img = (d2nn_img.clip(0, 1) * 255).byte().cpu().permute(0, 2, 3, 1).numpy()

        pil_images = [Image.fromarray(image.squeeze()) for image in d2nn_img]

        image_grid = make_image_grid(pil_images, rows=int(np.sqrt(config.test_batch_size)), 
                                                 cols=int(np.sqrt(config.test_batch_size)))
        image_grid.save(os.path.join(config.output_dir, 'example_celeba.png'))
